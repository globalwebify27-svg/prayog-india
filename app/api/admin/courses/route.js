...
export async function POST(req) {
  try {
    ...
    const { title, description, price, type, duration, image, teacher_id, selectedTimings, selectedSpecializations, allow_partial_payment, installments_count, rating, level, category, brochure, is_internship, is_one_to_one, outcomes, certification, who_can_join, methodology, important_note, upcoming_start_dates, custom_timings } = await req.json();

    const [result] = await pool.query(
      "INSERT INTO courses (title, description, price, type, duration, image, teacher_id, allow_partial_payment, installments_count, rating, level, category, brochure, is_internship, is_one_to_one, outcomes, certification, who_can_join, methodology, important_note, upcoming_start_dates, custom_timings) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [title, description, price, type, duration, image, teacher_id || null, allow_partial_payment ? 1 : 0, installments_count || 1, rating || "4.5", level || "Beginner", category || "Robotics", brochure || null, is_internship ? 1 : 0, is_one_to_one ? 1 : 0, outcomes || null, certification || null, who_can_join || null, methodology || null, important_note || null, upcoming_start_dates || null, custom_timings || null]
    );
...
export async function PUT(req) {
  try {
    ...
    const { id, title, description, price, type, duration, image, teacher_id, selectedTimings, selectedSpecializations, allow_partial_payment, installments_count, rating, level, category, brochure, is_internship, is_one_to_one, outcomes, certification, who_can_join, methodology, important_note, upcoming_start_dates, custom_timings } = await req.json();

    await pool.query(
      "UPDATE courses SET title = ?, description = ?, price = ?, type = ?, duration = ?, image = ?, teacher_id = ?, allow_partial_payment = ?, installments_count = ?, rating = ?, level = ?, category = ?, brochure = ?, is_internship = ?, is_one_to_one = ?, outcomes = ?, certification = ?, who_can_join = ?, methodology = ?, important_note = ?, upcoming_start_dates = ?, custom_timings = ? WHERE id = ?",
      [title, description, price, type, duration, image, teacher_id || null, allow_partial_payment ? 1 : 0, installments_count || 1, rating, level, category, brochure || null, is_internship ? 1 : 0, is_one_to_one ? 1 : 0, outcomes || null, certification || null, who_can_join || null, methodology || null, important_note || null, upcoming_start_dates || null, custom_timings || null, id]
    );
