...
export async function generateCertificate(studentName, courseName, certificateId, customBaseUrl) {
  try {
    ...
    // ── Logo ──────────────────────────────────────────────────────────
    const certLogoPath = path.join(process.cwd(), "public", logoUrl);
    let certLogoBase64 = null;
    if (fs.existsSync(certLogoPath)) {
      certLogoBase64 = fs.readFileSync(certLogoPath).toString("base64");
    }
    ...
    const baseUrl = customBaseUrl || process.env.NEXT_PUBLIC_BASE_URL || "https://prayogindiarobotics.com";
    ...
    // Use logo if available, else fallback to text
    if (certLogoBase64) {
      doc.addImage(certLogoBase64, "PNG", 340, 60, 120, 40);
    } else {
    ...
    doc.setFontSize(20);
    doc.setFont("helvetica", "bold");
    const wrappedCourse = doc.splitTextToSize(`"${courseName}"`, 600);
    const courseY = wrappedCourse.length > 1 ? 370 : 380;
    doc.text(wrappedCourse, 400, courseY, { align: "center" });

    // Bottom Left QR Code
    doc.addImage(qrDataUrl, "PNG", 50, 420, 80, 80);
    doc.setFontSize(10);
    doc.setTextColor(100, 116, 139);
    doc.text(`Certificate ID: ${certificateId}`, 90, 510, { align: "center" });
    doc.text("Scan to verify", 90, 525, { align: "center" });
...
export async function generateReceipt(data) {
  ...
  // ── Logo ──────────────────────────────────────────────────────────
  // Fetch logo_url from site_settings for dynamic logo support
  let logoBase64 = null;
  try {
    const [settingRows] = await pool.query("SELECT `setting_key`, `setting_value` FROM site_settings WHERE `setting_key` = 'logo_url'");
    const logoUrl = settingRows[0]?.setting_value || "/assets/logo.png";
...
export async function generateIDCard(studentName, rollNo, courseName, studentPhoto, idCardId, customBaseUrl) {
  const doc = new jsPDF({
    unit: "px",
    format: [250, 400]
  });
  ...
  const wrappedCourse = doc.splitTextToSize(courseName, 210);
  doc.text(wrappedCourse, 125, 260, { align: "center" });
