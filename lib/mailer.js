...
const smtpFrom = process.env.SMTP_FROM || process.env.SMTP_USER || "info@prayogindiarobotics.com";

const transporter = nodemailer.createTransport({
  pool: true,
  host: process.env.SMTP_HOST || "smtp.hostinger.com",
  port: smtpPort,
  secure: smtpPort === 465, // true for 465 (SSL), false for other ports like 587 (TLS)
  auth: {
    user: process.env.SMTP_USER || "your@email.com",
    pass: process.env.SMTP_PASS || "password",
  },
  maxConnections: 2, // Zoho/SMTP typical limit is very strict, keeping it safe at 2
  maxMessages: 100,  // Recreate connection after 100 messages
  rateLimit: 3,      // Limit to 3 messages per second to avoid triggering spam filters/rate limits
});

export async function sendMail(to, subject, html, attachments = []) {
  try {
    const info = await transporter.sendMail({
      from: `"Prayog India" <${smtpFrom}>`,
      to,
      subject,
      html,
      attachments
...
export const getOnboardingEmailTemplate = (studentName, email, password, courseName, timing, paymentMethod, amount, installments = [], logoUrl = "https://prayogindiarobotics.com/assets/logo.png") => {
  ...
  return `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
      <div style="background-color: #0f172a; padding: 30px 20px; text-align: center;">
        <img src="${logoUrl}" alt="Prayog India" style="height: 50px; margin-bottom: 10px; max-width: 250px; object-fit: contain;">
...
export const getFacultyOnboardingEmailTemplate = (teacherName, email, password, role, logoUrl = "https://prayogindiarobotics.com/assets/logo.png") => {
  return `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
      <div style="background-color: #0f172a; padding: 40px 20px; text-align: center;">
        <img src="${logoUrl}" alt="Prayog India" style="height: 50px; margin-bottom: 10px; max-width: 250px; object-fit: contain;">
...
export const getOTPEmailTemplate = (otp, duration = "10 minutes", logoUrl = "https://prayogindiarobotics.com/assets/logo.png") => {
  return `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
      <div style="background-color: #0f172a; padding: 30px 20px; text-align: center;">
        <img src="${logoUrl}" alt="Prayog India" style="height: 40px; margin-bottom: 10px; max-width: 200px; object-fit: contain;">
        <p style="color: #fbbf24; margin: 5px 0 0 0; font-weight: bold; text-transform: uppercase; font-size: 11px; letter-spacing: 2px;">Email Verification</p>
      </div>
      <div style="padding: 40px 30px; text-align: center;">
        <h2 style="color: #1e293b; font-size: 24px; margin-bottom: 10px; font-weight: 700;">Verify Your Email Address</h2>
        <p style="color: #475569; font-size: 15px; line-height: 1.6; max-width: 480px; margin: 0 auto 30px auto;">
          Thank you for starting your registration with Prayog India. Use the verification code below to complete your enrollment.
        </p>

        <div style="background-color: #f1f5f9; border-radius: 12px; padding: 20px; display: inline-block; min-width: 200px; border: 1px solid #cbd5e1; margin-bottom: 30px;">
          <span style="font-family: monospace; font-size: 32px; font-weight: 800; letter-spacing: 6px; color: #0f172a;">${otp}</span>
        </div>

        <p style="color: #64748b; font-size: 13px; margin-bottom: 0;">
          This code is valid for <strong>${duration}</strong>. Please do not share this OTP with anyone.
        </p>
      </div>
      <div style="background-color: #f8fafc; padding: 20px 30px; text-align: center; border-top: 1px solid #e2e8f0;">
        <p style="color: #94a3b8; font-size: 12px; margin: 0;">
